#!/usr/bin/env python3
"""Asset Generator CLI - creates images (Gemini / xAI Grok), Kie videos, and GLBs (Tripo3D).

Subcommands:
  image      Generate a PNG from a prompt (Gemini 5-15¢ or Grok 2¢)
  video      Generate MP4 via Kie.ai Grok Imagine (I2V if --image, else T2V)
  kie-status Check KIE_API_KEY + remaining credits (no generation)
  ...
  resume     Resume a timed-out Kie video or Tripo3D job from its sidecar — no extra cost
...
from kie_video import (
    ...
    resolve_image_url,
    resume_video,
    sidecar_path as kie_sidecar_path,
    write_sidecar as kie_write_sidecar,
)
from tripo3d import (
...
def result_json(ok: bool, path: str | None = None, cost_cents: int = 0, error: str | None = None, **extra):
...
def cmd_video(args):
    output = Path(args.output)
    if args.image and args.task_id:
        result_json(False, error="Pass either --image or --task-id, not both")
        sys.exit(1)
    if not args.image and not args.task_id:
        print("No --image / --task-id: text-to-video", file=sys.stderr)
    print(f"Generating {args.duration}s Kie video ({args.resolution})...", file=sys.stderr)
    try:
        ...
        task_id = None
        try:
            from kie_video import read_sidecar
            task_id = read_sidecar(output).get("taskId")
        except Exception:
            pass
        result_json(False, error=str(e), task_id=task_id)
        sys.exit(1)
    except (KieError, Exception) as e:
        result_json(False, error=str(e))
        sys.exit(1)
...
MODEL_T2I = "grok-imagine/text-to-image"
MODEL_I2I = "grok-imagine/image-to-image"


def _kie_image_job(args, model: str, kind: str):
    output = Path(args.output)
    output.parent.mkdir(parents=True, exist_ok=True)
    ...
    if kind == "i2i":
        ...
        if not image_path.exists():
            ...
            sys.exit(1)
        payload["image"] = resolve_image_url(image_path)
    kie_write_sidecar(output, {"kind": kind, "model": model, "status": "pending"})
    try:
        task_id = create_task(model, payload)
    except KieError as e:
        kie_write_sidecar(output, {"kind": kind, "model": model, "status": "unknown_submission", "error": str(e)})
        result_json(False, error=str(e))
        sys.exit(1)
    kie_write_sidecar(output, {"kind": kind, "model": model, "status": "running", "taskId": task_id})
    try:
    ...
    kie_write_sidecar(output, {"kind": kind, "model": model, "status": "complete", "taskId": task_id})
    print(f"Saved: {output}", file=sys.stderr)
    result_json(True, path=str(output), task_id=task_id, model=model)


def cmd_t2i(args):
    print(f"Kie T2I ({MODEL_T2I})...", file=sys.stderr)
    _kie_image_job(args, MODEL_T2I, "t2i")


def cmd_i2i(args):
    print(f"Kie I2I ({MODEL_I2I})...", file=sys.stderr)
    _kie_image_job(args, MODEL_I2I, "i2i")


def cmd_kie_status(_args):
    try:
    ...
    try:
        ...
        result_json(True, key=mask_key(key), credits=None, credits_error=str(e))
        return
    result_json(True, key=mask_key(key), credits=credits)
...
def cmd_resume(args):
    output = Path(args.output)
    if kie_sidecar_path(output).exists():
        try:
            result = resume_video(output)
        except TimeoutError as e:
...
def main():
    parser = argparse.ArgumentParser(description="Asset Generator — images, Kie video, Tripo3D GLBs")
    ...
    p_vid = sub.add_parser("video", help="Generate MP4 via Kie.ai Grok Imagine (I2V or T2V)")
    p_vid.add_argument("--prompt", required=True, help="Motion / scene prompt (max 5000 chars)")
    p_vid.add_argument("--image", default=None,
                       help="Pose PNG/JPEG/WebP path or public URL. Omit for text-to-video.")
    p_vid.add_argument("--task-id", dest="task_id", default=None,
                       help="Prior grok-imagine/text-to-image task id (instead of --image)")
    p_vid.add_argument("--index", type=int, default=0, help="Which Grok still to use with --task-id (0-5)")
    p_vid.add_argument("--duration", type=int, default=6, help="Seconds, 6-30. Default: 6")
    p_vid.add_argument("--resolution", choices=["480p", "720p", "1080p"], default="720p",
                       help="Video resolution. Default: 720p")
    p_vid.add_argument("--mode", choices=["fun", "normal", "spicy"], default="normal",
                       help="Motion style. spicy needs --task-id, not a local file.")
    p_vid.add_argument("--aspect-ratio", default=None, choices=["1:1", "16:9", "9:16", "2:3", "3:2"],
                       help="T2V / multi-image only. Single-image I2V follows the pose.")
    p_vid.add_argument("-o", "--output", required=True, help="Output MP4 path")
    p_vid.set_defaults(func=cmd_video)

    p_kie = sub.add_parser("kie-status", help="Check KIE_API_KEY and remaining credits (no generation)")
    p_kie.set_defaults(func=cmd_kie_status)

    p_t2i = sub.add_parser("t2i", help="Generate PNG via Kie grok-imagine/text-to-image")
    p_t2i.add_argument("--prompt", required=True)
    p_t2i.add_argument("--aspect-ratio", default="1:1")
    p_t2i.add_argument("-o", "--output", required=True)
    p_t2i.set_defaults(func=cmd_t2i)

    p_i2i = sub.add_parser("i2i", help="Generate PNG via Kie grok-imagine/image-to-image")
    p_i2i.add_argument("--prompt", required=True)
    p_i2i.add_argument("--image", required=True, help="Reference image path")
    p_i2i.add_argument("--aspect-ratio", default="1:1")
    p_i2i.add_argument("-o", "--output", required=True)
    p_i2i.set_defaults(func=cmd_i2i)
    ...
    p_res = sub.add_parser("resume", help="Resume a timed-out Kie video or Tripo3D job from its sidecar (no extra cost)")
    p_res.add_argument("-o", "--output", required=True,
                       help="Output path whose .kie.json or .tripo.json sidecar holds the pending task id")
